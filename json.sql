...
SELECT JSON_OBJECT(
    ...
    'buyers', CAST(IFNULL(pb.buyers_array, JSON_ARRAY()) AS JSON)
)
...
SELECT 
    ...
    JSON_ARRAYAGG(
        JSON_OBJECT(
            ...
            'items', CAST(IFNULL(oi.items_array, JSON_ARRAY()) AS JSON)
        )
    ) AS orders_array
FROM `Order` o
...
SELECT JSON_OBJECT(
    ...
    'orders', CAST(IFNULL(co.orders_array, JSON_ARRAY()) AS JSON)
)
...
SELECT JSON_OBJECT(
    ...
    'city', ci.city,
    'state', ci.state,
    'resident_customers', CAST(IFNULL(zc.cust_array, JSON_ARRAY()) AS JSON)
)
...
SELECT JSON_OBJECT(
    ...
    'sales_history', CAST(IFNULL(ph.sales_history, JSON_ARRAY()) AS JSON)
)
