...
SELECT JSON_OBJECT(
    ...
    'buyers', JSON_QUERY(JSON_EXTRACT(IFNULL(pb.buyers_array, '[]'), '$'))
)
)
...
SELECT 
    ...
    JSON_ARRAYAGG(
        JSON_OBJECT(
            ...
            'items', JSON_QUERY(JSON_EXTRACT(IFNULL(oi.items_array, '[]'), '$'))
        )
    ) AS orders_array
FROM `Order` o
...
SELECT JSON_OBJECT(
    ...
    'orders', JSON_QUERY(JSON_EXTRACT(IFNULL(co.orders_array, '[]'), '$'))
)
...
SELECT JSON_OBJECT(
    ...
    'city', ci.city,
    'state', ci.state,
      'resident_customers', JSON_QUERY(JSON_EXTRACT(IFNULL(zc.cust_array, '[]'), '$'))
)
)
...
SELECT JSON_OBJECT(
    ...
    'sales_history', JSON_QUERY(JSON_EXTRACT(IFNULL(ph.sales_history, '[]'), '$'))
)
